...
def getAllResults():
    ...
    result = session.query(Session2,Member,Role,Role_Type, Club).join(Member,Session2.member_id==Member.member_id).join(Role,Session2.role_id==Role.role_id).join(Role_Type,Role.role_type_id==Role_Type.role_type_id).join(Club,Member.club_id==Club.club_id).all()
    dicts = []
    for row in result:
        d = {}
...
def getResultsPerMember(member_):
    engine = create_engine(connection)
    ...
    result = session.query(Session2,Member,Role,Role_Type, Club).join(Member,Session2.member_id==Member.member_id).join(Role,Session2.role_id==Role.role_id).join(Role_Type,Role.role_type_id==Role_Type.role_type_id).join(Club,Member.club_id==Club.club_id).filter(Member.member_desc == member_).all()
    dicts = []
    for row in result:
        d = {}
        print(row.Session.session_dt, row.Role.role_desc, row.Role.role_desc, row.Role_Type.role_type_desc, row.Club.club_desc)
    ...
    results = results.groupby(by=["session_dt","member_desc","role_desc"]).agg({'role_type_id':'count'})
    results = results.rename(columns={'role_type_id':'NoParticipations'})
    return results

def getResultsPerRole(role_):
    engine = create_engine(connection)
...
def getResultsPerRoleType(role_type_):
    engine = create_engine(connection)
...
def getResultsPerSession(date_):
    engine = create_engine(connection)
